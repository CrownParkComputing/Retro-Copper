...
/* A host with no networking at all: the serial and parallel ports cannot be
 * served over TCP, so every listener fails to open and nothing here touches
 * the socket API. The URI helper below is shared and stays. */
bool uae_socket_init(void) { return false; }
int uae_socket_error(void) { return 0; }
uae_socket uae_tcp_listen(const TCHAR *, const TCHAR *, int) { return UAE_SOCKET_INVALID; }
uae_socket uae_tcp_listen_uri(const TCHAR *, const TCHAR *, int) { return UAE_SOCKET_INVALID; }
uae_socket uae_socket_accept(uae_socket) { return UAE_SOCKET_INVALID; }
int uae_socket_read(uae_socket, void *, int) { return -1; }
int uae_socket_write(uae_socket, const void *, int) { return -1; }
int uae_socket_select(uae_socket, bool, bool, bool, uae_u64) { return UAE_SELECT_ERROR; }
bool uae_socket_close(uae_socket) { return true; }
#else /* UAE_NO_NETWORK */
